    ...
    where TContext : DbContext
{
    ...
    public async Task<TEntity> DeleteAsync(TEntity entity, bool permanent = false) // Permanent = kalıcı mı?
    {
    ...
    public async Task<TEntity> GetAsnyc(Expression<Func<TEntity, bool>> predicate, Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>> include = null, bool withDeleted = false, bool enableTracking = true, CancellationToken cancellationToken = default)
    {
        IQueryable<TEntity> queryable = Query();
        if (!enableTracking)
            queryable = queryable.AsNoTracking();
    ...
    protected async Task SetEntityAsDeletedAsync(TEntity entity, bool permanent)
    {
        if (!permanent)
        {
            CheckHasEntityHaveOneToOneRelation(entity); // one to one birebir ilişkisi var mı
